...
CREATE TABLE book_instances (
    ...
    barcode VARCHAR(100) UNIQUE, -- 바코드는 고유해야 함
    notes TEXT,
    created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE book_analyses (
    ...
    created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT check_hexagon_value_1_range CHECK (hexagon_value_1 IS NULL OR (hexagon_value_1 >= 0 AND hexagon_value_1 <= 10)),
    CONSTRAINT check_hexagon_value_2_range CHECK (hexagon_value_2 IS NULL OR (hexagon_value_2 >= 0 AND hexagon_value_2 <= 10)),
    CONSTRAINT check_hexagon_value_3_range CHECK (hexagon_value_3 IS NULL OR (hexagon_value_3 >= 0 AND hexagon_value_3 <= 10)),
    CONSTRAINT check_hexagon_value_4_range CHECK (hexagon_value_4 IS NULL OR (hexagon_value_4 >= 0 AND hexagon_value_4 <= 10)),
    CONSTRAINT check_hexagon_value_5_range CHECK (hexagon_value_5 IS NULL OR (hexagon_value_5 >= 0 AND hexagon_value_5 <= 10)),
    CONSTRAINT check_hexagon_value_6_range CHECK (hexagon_value_6 IS NULL OR (hexagon_value_6 >= 0 AND hexagon_value_6 <= 10))
);
...
-- Table: book_analyses (도서 분석 - 6각형값, 평점, 서평 등)
CREATE TABLE book_analyses (
    analysis_id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, -- SERIAL 대신 IDENTITY 사용
    book_id INTEGER NOT NULL REFERENCES books(book_id) ON DELETE CASCADE UNIQUE,
